...
def parameter(id=None):
    if request.method == "POST":
        if request.form["id"] == "":
            add_parameter(
                request.form["parameter_desc"], request.form["deviation_desc"]
            )
        else:
            update_parameter(
                request.form["parameter_desc"],
                request.form["deviation_desc"],
                int(request.form["id"]),
            )
...
def unit():
    if request.method == "POST":
        ...
        units = get_units()
        unit_types = get_unit_types()
        return render_template("settings/unit.html", units=units, unit_types=unit_types)


@settings_bp.route("/document_types", methods=["GET", "POST"])
def document_type(id=None):
    if request.method == "POST":
        if request.form["id"] == "":
            ...
            update_document_type(
                request.form["document_type_desc"], int(request.form["id"])
            )
        return redirect(url_for(".document_type"))
    else:
        document_types = get_document_types()
        return render_template(
            "/settings/document_type.html",
